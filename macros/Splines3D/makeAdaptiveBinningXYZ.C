...
void makeAdaptiveBinningXYZ(const char* input_file, const char* output_file, int targetx=10000000, int target=500000, bool isX=false, int axis=0) {
    ...
    for (int tpc = 0; tpc < 2; tpc++) {
      for (int plane = 0; plane < 3; plane++) {
        ...
        // First --> Profile over X and make X-bins
        TH1D* hx = hTrk->Projection(0);
        std::vector<float> x_bin_edges;

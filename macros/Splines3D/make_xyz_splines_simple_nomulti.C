                                ...
                                const char* bin_file, int IDX)
{
    ...
    for (Long64_t i = 0; i < nentries; ++i) {
        ...
        regions.push_back({(int)idx, (int)X, (int)x1, (int)x2, (int)y1, (int)y2});
    }
    std::cout << "Preloaded " << regions.size() << " regions." << std::endl;

    // Parallel section
    std::mutex result_mutex;
    std::vector<std::vector<std::tuple<int, double,double,double,double,double,double,double>>> results(1);
    ...
    auto process_region = [&](const BinRegion& R) {
        ...
        std::cout << "Got MC and Data ND Hists ..." <<std::endl;

        std::cout << "Current X bin " << R.X << std::endl;
    ...
    std::cout << "Done multi-processing" << std::endl;
    ...
    for (auto& r : results[0]) {
      ...
      if (std::find(Xvals.begin(), Xvals.end(), X) == Xvals.end()) {
        Xvals.push_back(X);
      }
    }
    size_t Nfit = Xvals.size();
    // Merge results into graphs
    TFile outfile(output_file, "RECREATE");
    //TGraph2DErrors* mc_2d_fits[Nfit];
    //TGraph2DErrors* data_2d_fits[Nfit];
    //TGraph2DErrors* splines_2d[Nfit];

    std::vector<TGraph2DErrors*> mc_2d_fits;
    std::vector<TGraph2DErrors*> data_2d_fits;
    std::vector<TGraph2DErrors*> splines_2d;
    
    std::unique_ptr<TFile> f(TFile::Open(mc_file, "READ"));
    auto h = (THnSparseD*)f->Get(Form("hHit%d", 0));
    if (!h) return;
    auto haxes = h->Projection(2, 1);
    std::cout << "X-axis title " << haxes->GetXaxis()->GetTitle() << std::endl;
    std::cout << "Y-axis title " << haxes->GetYaxis()->GetTitle() << std::endl;
    std::cout << "Initialize Graphs" << std::endl;
    for (int i = 0; i < Xvals.size(); ++i) {
        mc_2d_fits.push_back( new TGraph2DErrors() );
        data_2d_fits.push_back( new TGraph2DErrors() );
        splines_2d.push_back( new TGraph2DErrors() );

        mc_2d_fits[i]->SetName(Form("mc_mpv_%d_%d", IDX, Xvals[i]));
        data_2d_fits[i]->SetName(Form("data_mpv_%d_%d", IDX, Xvals[i]));
        splines_2d[i]->SetName(Form("splines_%d_%d", IDX, Xvals[i]));
     ...
     for (auto& r : results[IDX]) {
            ...
            double x, y, mpv_mc, mpv_data, ratio, err_mc, err_data;
            std::tie(X, x, y, mpv_mc, mpv_data, ratio, err_mc, err_data) = r;
            size_t index = std::find(Xvals.begin(), Xvals.end(), X) - Xvals.begin();

            int n1 = mc_2d_fits[index]->GetN();
            mc_2d_fits[index]->SetPoint(n1, x, y, mpv_mc);
            mc_2d_fits[index]->SetPointError(n1, 0, 0, err_mc);

            int n2 = data_2d_fits[index]->GetN();
            data_2d_fits[index]->SetPoint(n2, x, y, mpv_data);
            data_2d_fits[index]->SetPointError(n2, 0, 0, err_data);

            int n3 = splines_2d[index]->GetN();
            splines_2d[index]->SetPoint(n3, x, y, ratio);
            splines_2d[index]->SetPointError(n3, 0, 0, 0);
     }
